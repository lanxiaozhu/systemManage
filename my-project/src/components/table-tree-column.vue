...
export default {
  ...
  methods: {
    childStyles (row) {
      return {
        'padding-left': (row.levelKey > 1 ? row.levelKey * 7 : 0) + 'px'
      }
    },
    iconClasses (row) {
    ...
    hasChild (row) {
      return (Array.isArray(row.menuList) && row.menuList.length >= 1) || false
    },
    // 切换处理
    toggleHandle (index, row) {
      if (this.hasChild(row)) {
        ...
        if (data[index]._expanded) {
          data = data.splice(0, index + 1).concat(row.menuList).concat(data)
        } else {
